...
abstract class Helpers {
	...
	public static function get_shares_list() {
		...
		if ( ! empty( $shares_posts ) ) {
			foreach ( $shares_posts as $share ) {
				$share_name = get_post_meta( $share->ID, 'name', true ) ?? 0;
				$share_symbol = get_post_meta( $share->ID, 'symbol', true ) ?? 0;
	...
	public static function reset_all_lab_data() {
		...
		printf( '<div class="notice notice-success is-dismissible"><h4>Data Reset Successfu!ly!</h4><p>%s</p></div>', $posts_count );
	}
}
